...
class CMPPSocket extends EventEmitter {
  constructor(config) {
    ...
    // TCP 连接是否就绪
    this.isReady = false
    // CMPP 鉴权是否已通过, 也是发送 CMPP 数据的入口
    this.cmppConnected = false
  ...
  handleHeartbeat() {
    ...
    if (this.isReady) {
      ...
      this.send(cmdCfg.Commands.CMPP_ACTIVE_TEST, this.config.heartbeatTimeout).then(function () {
  ...
  connect(port, host) {
    ...
    return this.connectSocket(port, host).then(() => {
      ...
      _this.destroySocket(_this.socket)
    })
  }
  ...
  connectSocket(port, host) {
    ...
    this.socket.on('connect', function () {
        _this.debug('socket connected');
        _this.emit('connect')
        deferred.resolve()
    })
    this.socket.connect(port, host)
    this.socket.on('close', function() {
        _this.debug('socket close');
    });
  ...
  disconnect() {
    ...
    // 关闭鉴权即关闭发送提交
    this.cmppConnected = false
    // 心跳要在 CMPP_TERMINATE 之前断开，因为服务器端会断开
    clearTimeout(this.heartbeatHandle)
    return this.send(cmdCfg.Commands.CMPP_TERMINATE, 3 * 1000).catch(function () { }).finally(function () {
      _this.destroySocket(_this.socket)
    })
  }
  ...
  fetchData(obj) {
    ...
    this.debug('%s receive buffer: ', cmdCfg.Commands[obj.header.Command_Id] || obj.header.Command_Id, obj.buffer);
  ...
  sendResponse(command, sequence, body) {
    ...
    this.debug('%s send buffer:', cmdCfg.Commands[command] || command, buf.inspect())
   ...
   * @param {?object} body
   * @param {?number} [specified_timeout = -1] 指定超时时间(milliseconds)
   * @returns
   */
  send(command, body, specified_timeout = -1) {
    if (typeof body === 'number') {
      specified_timeout = body;
      body = null;
    }
    const _this = this
