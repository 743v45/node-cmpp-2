...
class Client extends EventEmitter {
  ...
  connect(spId, secret) {
    ...
    return this.socket.connect(this.config.port, this.config.host).then((connected) => {
      // TCP 已经连接
      if (connected) {
        return this.socket.send(cmdCfg.Commands.CMPP_CONNECT, {
          ...
          // 对于3.0版本CMPP，要求CMPP_CONNECT消 息和CMPP_CONNECT_RESP消息中的Version字 段表示3.0版本;
  ...
  bindEvent() {
     ...
     */
    this.socket.on('deliver', (data, sendRes) => {
